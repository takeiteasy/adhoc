...
- TODO: whether `true`/`false` are `\true`/`\false` or bare literals is undecided — there is no boolean type until comparison operators land, so this is deferred alongside them.
